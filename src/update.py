...
class LocalUpdate(object):
    def __init__(
        ...
        if dataset is not None:
            if self.args.ssl_method == "pu":
                self.trainloader, self.validloader, self.testloader, self.idxloader = self.train_val_test(
    ...
    def train_val_test(self, dataset, idxs, test_dataset=None, memory_dataset=None):
        ...
        if self.args.ssl_method == "pu":
            from clu import dataset_with_indices
            idxloader = DataLoader(
        ...
        #     idx_dataset = DatasetSplit(
        #     memory_dataset,
        #     idxs_train,
        #     idx=self.id,
        # )
        #     idx_dataset = dataset_with_indices(idx_dataset)
        #     self.idxloader = DataLoader(
        #         idx_dataset,
        #         batch_size=64,
        #         shuffle=False,
        #         num_workers=1,
        ...
        if not self.args.distributed_training:
            ...
            if self.args.ssl_method == "pu":
                trainloader = DataLoader(
                    dataset_with_indices(train_dataset),
        ...
        if self.args.ssl_method == "pu":
            return trainloader, validloader, testloader, idxloader
        else:
    ...
    def update_ssl_weights(
        ...
        elif self.args.optimizer == "pid":
            train_lr = lr if lr is not None else self.args.lr
            optimizer = AdamPIDOptimizer(
                model.parameters(), lr=train_lr, P=float(0.5), I=3, D=6
            )
        ...
        if self.args.ssl_method == "pu":
            from clu import extract_features
            from clu import clustering
            model_encoder = model.module.get_encoder().to(self.device)
            mem_features, mem_labels = extract_features(model_encoder, self.idxloader)
            mem_features = F.normalize(mem_features, dim=1)
            psedo_labels, cluster_centers = clustering(mem_features, 10)
            ...
            for batch_idx, data in enumerate(self.trainloader):
                ...
                if additionl_feature_banks is not None:
                    ...
                    loss, feat = model(
                        ...
                        addition_features,#应该是后续模型中的距离度量，具体可见模型设计
                        self.reg_scale,
                        return_feat=True,
                    )
                else:
                    ...
                    elif self.args.ssl_method == "pu":
                        (data_local, indices) = data
                        (pos_1, pos_2, labels) = data_local
                        pred_labels = psedo_labels[indices]
                        loss, feat = model(
                            pos_1.to(self.device, non_blocking=True),
                            pos_2.to(self.device, non_blocking=True),
                            predo_label=pred_labels,
                            return_feat=True,
                        )
                    else:
                        (pos_1, pos_2, labels) = data
    ...
    def update_weights(
        ...
        if self.args.optimizer == "sgd":
            ...
            optimizer = AdamPIDOptimizer(
                model.parameters(), lr=self.args.lr, P=float(0.5), I=3, D=6
            )
        if self.args.model_continue_training and hasattr(self, "optimizer"):
            optimizer.load_state_dict(self.optimizer.state_dict())
